...
const SOURCE_OPTIONS = [
  ...
  { value: 'cold_call', label: 'Cold Call' }, { value: 'social', label: 'Social' },
...
export function LeadFormPage() {
  ...
  useEffect(() => {
    if (lead?.data) {
      ...
      setForm({
        ...
        email: d.contact.email, phoneNumber: d.contact.phoneNumber,
        companyName: d.contact.companyName ?? '', website: d.contact.website ?? '',
        currency: d.contact.currency ?? 'NGN',
        source: d.source,
        assignedTo: typeof d.assignedTo === 'object' && d.assignedTo ? d.assignedTo._id : (d.assignedTo ?? ''),
        tags: d.tags.join(', '), nextFollowUpAt: d.nextFollowUpAt?.slice(0, 10) ?? '',
  ...
  const mutation = useMutation({
    mutationFn: () => {
      const body = {
        contact: { type: form.type, firstName: form.firstName, lastName: form.lastName, email: form.email, phoneNumber: form.phoneNumber, companyName: form.type === 'company' ? form.companyName : '', website: form.type === 'company' ? form.website : '', currency: form.currency },
    ...
    onSuccess: () => { toast.success(isEdit ? 'Lead updated' : 'Lead created'); qc.invalidateQueries({ queryKey: ['crm-leads'] }); qc.invalidateQueries({ queryKey: ['crm-prospects'] }); navigate(basePath); },
  ...
  return (
    <PageShell title={isEdit ? 'Edit lead' : 'New lead'} maxWidth="max-w-2xl">
      <form onSubmit={e => { e.preventDefault(); mutation.mutate(); }} className="space-y-5">
        <SCard title="Contact info">
          <div className="space-y-4">
            <Field.Select label="Type" options={[{ value: 'individual', label: 'Individual' }, { value: 'company', label: 'Company' }]} value={form.type} onChange={set('type')} />
            ...
            <Field label="Phone *"          required value={form.phoneNumber}  onChange={set('phoneNumber')} />
            {form.type === 'company' && (
              <>
                <Field label="Company name *" required value={form.companyName} onChange={set('companyName')} />
                <Field label="Website *" required value={form.website} onChange={set('website')} />
              </>
            )}
        ...
        <SCard title="Lead details">
          <div className="space-y-4">
            <Field.Select label="Source" options={SOURCE_OPTIONS} value={form.source} onChange={set('source')} />
            <Field.Select label="Assigned to" options={userOptions} value={form.assignedTo} onChange={set('assignedTo')} />
            <Field label="Tags (comma-separated)" value={form.tags} onChange={set('tags')} placeholder="saas, q3" />
            <Field label="Next follow-up" type="date" value={form.nextFollowUpAt} onChange={set('nextFollowUpAt')} />
            <Field label="Expected interest" value={form.expectedInterest} onChange={set('expectedInterest')} placeholder="Annual ERP license" />
          </div>
        </SCard>

        <div className="flex gap-3">
          <Btn type="submit" loading={mutation.isPending}>{isEdit ? 'Update lead' : 'Create lead'}</Btn>
          <Btn type="button" variant="secondary" onClick={() => navigate(basePath)}>Cancel</Btn>
        </div>
      </form>
    </PageShell>
  );
